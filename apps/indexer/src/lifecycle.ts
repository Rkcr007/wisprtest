 ...
 * thing it depends on. A hook that throws is recorded and the drain continues.
 *
 * Deliberately per-service rather than shared: the gateway drains in-flight HTTP requests and
 * connection pools, while the indexer must finish or requeue the crawl job it holds and close
 * a browser context. These diverge in Phases 4 and 5, and CLAUDE.md § "Repository layout"
 * has no package for cross-service runtime code.
 */
...
export class Lifecycle {
  ...
  /** Begins the drain, or returns the in-flight one, so a second signal runs no hook twice. */
